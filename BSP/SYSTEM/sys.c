 ...
 *              AF0~15设置情况(这里仅是列出常用的, 详细的请见STM32H743xx数据手册, Table 10~20):
...
{
    PWR->WKUPEPR &= ~(1 << 0);  /* WKUPEN1 = 0, PA0不用于WKUP唤醒 */
    PWR->WKUPEPR |= 1 << 0;     /* WKUPEN1 = 1, PA0用于WKUP唤醒 */
    PWR->WKUPEPR &= ~(1 << 8);  /* WKUPP1 = 0, PA0高电平唤醒(上升沿) */
    PWR->WKUPEPR &= ~(3 << 16); /* 清除WKUPPUPD1原来的设置 */
    PWR->WKUPEPR |= 2 << 16;    /* WKUPPUPD1 = 10, PA0下拉 */
    PWR->WKUPCR |= 0X3F << 0;   /* 清除所有WKUP唤醒引脚标志 */
    PWR->CPUCR |= 7 << 0;       /* PDDS_D1/D2/D3 = 1, 在D1/D2/D3域进入深度睡眠时允许DStandby模式 */
...
uint8_t sys_clock_set(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    /* 480M版本H743芯片（V版本）新增Scale0设置,注意在进入低功耗模式之前, 必须先退出Scale0模式！！ */
    RCC->APB4ENR |= 1 << 1;     /* 使能SYSCFG时钟 */
    SYSCFG->PWRCR |= 1 << 0;    /* 设置ODEN位为1, 使能Overdrive模式, 此时进入Scale0模式, VCORE = 1.35V */
    ...
    else
    {
        ...
        FLASH->ACR |= 4 << 0;               /* LATENCY[3:0] = 4, 4个CPU等待周期(@VOS0 Level, maxclock = 240Mhz) */
        FLASH->ACR |= 2 << 4;               /* WRHIGHFREQ[1:0] = 2, 编程操作时Flash信号延迟为2 */
